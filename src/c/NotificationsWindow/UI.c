...
static void text_display_layer_paint(Layer* layer, GContext* ctx)
{
	...
	Notification* curNotification = nw_get_displayed_notification();
		...
		if(curNotification != NULL){
			//Color behind notification title
			graphics_context_set_fill_color(ctx, curNotification->notificationColor);
			
			graphics_fill_rect(ctx, GRect(0, title.bounds.origin.y, body.bounds.size.w + 4, title.bounds.size.h + 2), 0, GCornerNone);
			
			graphics_context_set_text_color(ctx, getTextColor(curNotification->notificationColor) );
		}
	#endif
...
void nw_ui_load(Window* window)
{
	...
	//Add content indicator for scrolling down
	s_indicator = scroll_layer_get_content_indicator(scroll);
   	s_indicator_down= layer_create(GRect(0, windowBounds.size.h - STATUS_BAR_LAYER_HEIGHT, windowBounds.size.w, STATUS_BAR_LAYER_HEIGHT));

	layer_add_child(topLayer, s_indicator_down);
